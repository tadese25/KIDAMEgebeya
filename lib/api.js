...
export async function getPromo(code) {
  const row = await db.get('SELECT * FROM promo_codes WHERE code = ? AND active = 1', String(code || '').trim().toUpperCase());
  if (!row) return null;
  return {
    code: row.code,
    type: row.type,
...
export async function computeTotals(items, promoCode) {
  const subtotal = items.reduce((s, i) => s + i.unit_price * i.qty, 0);
  const promo = promoCode ? await getPromo(promoCode) : null;
