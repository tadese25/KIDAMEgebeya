...
export async function issueToken({ userId, type, ttlHours }) {
  const token = generateToken();
  const expiresAt = new Date(Date.now() + ttlHours * 3600e3);
  await db.run(
...
export async function consumeToken({ token, type }) {
  const row = await db.get(
    `SELECT * FROM auth_tokens
      WHERE token_hash = ? AND type = ? AND consumed = 0 AND expires_at > now()`,
    hashToken(String(token || '')), type
  );
  if (!row) return null;
  await db.run('UPDATE auth_tokens SET consumed = 1 WHERE id = ?', row.id);
  return row;
}

export async function latestLiveToken({ userId, type }) {
  return db.get(
    `SELECT * FROM auth_tokens
      WHERE user_id = ? AND type = ? AND consumed = 0 AND expires_at > now()
...
export async function issueOtp({ userId, type, ttlMinutes, digits = 6 }) {
  ...
  await db.run('DELETE FROM auth_tokens WHERE user_id = ? AND type = ? AND consumed = 0', userId, type);
  const code = generateOtp(digits);
  const expiresAt = new Date(Date.now() + ttlMinutes * 60e3);
  await db.run(
    'INSERT INTO auth_tokens (user_id, type, token_hash, expires_at) VALUES (?,?,?,?)',
    userId, type, hashToken(code), expiresAt
  );
  return code;
}

export async function consumeOtp({ userId, type, code, maxAttempts = 5 }) {
  const row = await db.get(
    `SELECT * FROM auth_tokens
      WHERE user_id = ? AND type = ? AND consumed = 0 AND expires_at > now()
      ORDER BY id DESC LIMIT 1`,
  ...
  if (hashToken(String(code || '')) !== row.token_hash) {
    ...
    if (attempts >= maxAttempts) await db.run('DELETE FROM auth_tokens WHERE id = ?', row.id);
    else await db.run('UPDATE auth_tokens SET attempts = ? WHERE id = ?', attempts, row.id);
    return null;
  }
  await db.run('UPDATE auth_tokens SET consumed = 1 WHERE id = ?', row.id);
  return row;
